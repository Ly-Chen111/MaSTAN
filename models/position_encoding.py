...
class PositionEmbeddingSine1D(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # for pytorch<1.8, dont have the parameter rounding_mode, modify this function by socket trunc func individually
...
class PositionEmbeddingSine2D(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=x.device)
        # dim_t = self.temperature ** (2 * torch.div(dim_t, 2, rounding_mode='trunc') / self.num_pos_feats)
        # for pytorch<1.8, dont have the parameter rounding_mode, modify this function by socket floor func individually
        dim_t = self.temperature ** (2 * torch.trunc(torch.div(dim_t, 2)) / self.num_pos_feats)
