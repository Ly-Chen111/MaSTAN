...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], size_divisibility=1, split=True):
    ...
    if tensor_list[0].ndim == 3: 
        ...
        for img, pad_img, m in zip(tensor_list, tensor, mask):
            ...
            m[: img.shape[1], :img.shape[2], :img.shape[3]] = False # valid locations
    else:
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    """
    return [{k: v.to(device)  for k, v in t.items() if k not in ["caption", "dataset_name", "original_id", "image_id", "valid"]} for t in targets]
